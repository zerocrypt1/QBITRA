...
s := &models.Session{UserID: userID, RefreshJTI: refreshJTI, IPHash: hashPII(ip), UserAgentHash: hashPII(ua), ExpiresAt: time.Now().UTC().Add(s.cfg.JWTRefreshTTL)}
if err := s.sessions.Create(ctx, s); err != nil {
return "", "", err
}
